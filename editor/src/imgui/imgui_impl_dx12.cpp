...
void ImGui_ImplDX12_CreateFontSRVAtHandle(D3D12_CPU_DESCRIPTOR_HANDLE fontSRVCPUHandle, D3D12_GPU_DESCRIPTOR_HANDLE fontSRVGPUHandle)
{
    ImGuiIO& io = ImGui::GetIO();
    ImGui_ImplDX12_Data* bd = ImGui_ImplDX12_GetBackendData();
    // Create texture view
    ...
    bd->hFontSrvCpuDescHandle = fontSRVCPUHandle;
    bd->hFontSrvGpuDescHandle = fontSRVGPUHandle;
    bd->pd3dDevice->CreateShaderResourceView(bd->pFontTextureResource, &srvDesc, bd->hFontSrvCpuDescHandle);
